...
def get_random_prompt(topic: str, n_samples: int) -> str:
    prompts = [
        (
            ...
            f"The data should be structured as a JSON array, where each "
            f"entry has a \"text\" field, like this:\n"
            f"[\n"
        ...
        (
            f"Task: produce fine-tuning data for a language model focused "
            f"on {topic}.\n\n"
            f"Requirements:\n"
            f"- Return a JSON array (top-level list, no wrapper object).\n"
            f"- Each element is an object with a single key \"text\".\n"
            f"- Exactly {n_samples} elements.\n"
            f"- Every \"text\" value is a self-contained passage about "
            f"{topic}, drawing on real, concrete details.\n"
            f"- No two entries may overlap in sub-topic, technique, or "
            f"scenario.\n\n"
            f"Shape:\n"
            f"[\n"
            f"  {{\"text\": \"...\"}},\n"
        ...
        (
            ...
            f"The structure should be a JSON array, where each entry "
            f"has \"instruction\" and \"output\" fields, like this:\n"
            f"[\n"
